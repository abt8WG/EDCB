...
UINT CEpgDBManager::LoadThread()
{
	...
	__int64 arcMin = this->archivePeriodSec <= 0 ? LLONG_MAX : arcMax - this->archivePeriodSec * I64_1SEC;
	arcMax += 3600 * I64_1SEC;

	if( arcMin < LLONG_MAX && this->epgArchive.empty() ){
		vector<BYTE> buff;
		...
		if( buff.empty() == false ){
			...
			if( ReadVALUE(&ver, &buff.front(), (DWORD)buff.size(), &readSize) &&
				...
				for( size_t i = 0; i < list.size(); i++ ){
					...
					this->epgArchive[key] = std::move(list[i]);
				}
			}
		}
	}
	//アーカイブから古いイベントを消す
	for( auto itr = this->epgArchive.begin(); itr != this->epgArchive.end(); itr++ ){
	...
	//イベントをアーカイブに移動する
	for( auto itr = this->epgMap.begin(); arcMin < LLONG_MAX && itr != this->epgMap.end(); itr++ ){
		auto itrArc = this->epgArchive.find(itr->first);
		if( itrArc != this->epgArchive.end() ){
		...
		for( size_t i = 0; i < itr->second.eventList.size(); i++ ){
			if( itr->second.eventList[i].StartTimeFlag &&
				...
				if( itrArc == this->epgArchive.end() ){
					//サービスを追加
					itrArc = this->epgArchive.insert(std::make_pair(itr->first, EPGDB_SERVICE_EVENT_INFO())).first;
					itrArc->second.serviceInfo = std::move(itr->second.serviceInfo);
				}
	...
	//アーカイブから不要なイベントを消す
	for( auto itr = this->epgMap.cbegin(); arcMin < LLONG_MAX && itr != this->epgMap.end(); itr++ ){
		auto itrArc = this->epgArchive.find(itr->first);
		if( itrArc != this->epgArchive.end() ){
	...
	//アーカイブから空のサービスを消す
	for( auto itr = this->epgArchive.begin(); itr != this->epgArchive.end(); ){
		if( itr->second.eventList.empty() ){
			this->epgArchive.erase(itr++);
		}else{
			itr++;
		}
	}
	if( arcMin < LLONG_MAX ){
		...
		valp.reserve(this->epgArchive.size());
		for( auto itr = this->epgArchive.cbegin(); itr != this->epgArchive.end(); valp.push_back(&(itr++)->second) );
		DWORD buffSize;
