...
bool CEpgTimerSrvMain::ChgAutoAdd(vector<EPG_AUTO_ADD_DATA>& val) {
	...
	if (modified) {
		...
		notifyManager.AddNotify(NOTIFY_UPDATE_AUTOADD_EPG);
		if( addReserve ){
			reserveManager.AddNotifyAndPostBat(NOTIFY_UPDATE_RESERVE_INFO);
		}
	}
	return modified;
}
...
//{
//	vector<con*st EPGDB_EVENT_INFO*> valp;
//	valp.reserve(pval->size());
//	for( size_t i = 0; i < pval->size(); i++ ){
//		valp.push_back((*pval)[i].info);
//	}
...
int CEpgTimerSrvMain::CtrlCmdCallback(void* param, CMD_STREAM* cmdParam, CMD_STREAM* resParam, bool tcpFlag)
{
	...
	if( sys->CtrlCmdProcessCompatible(*cmdParam, *resParam, tcpFlag) ){
		// return 前に OutputDebugCmd() したいので
		//return 0;
	}
	else {
...
void CEpgTimerSrvMain::OutputDebugCmd(DWORD cmd, DWORD result, LONGLONG ticks)
{
	...
	case CMD2_EPG_SRV_ENUM_PG_ARC_INFO: strCmd = L"CMD2_EPG_SRV_ENUM_PG_ARC_INFO"; break;
	case CMD2_EPG_SRV_ENUM_PG_ARC_ALL: strCmd = L"CMD2_EPG_SRV_ENUM_PG_ARC_ALL"; break;
	...
	case CMD2_EPG_SRV_GET_RECINFO_LIST2: strCmd = L"CMD2_EPG_SRV_GET_RECINFO_LIST2"; break;
